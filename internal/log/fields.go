//
// Direct copy of https://github.com/DataDog/dd-source/blob/3093c762a21a3b05b0950b2fd1bd79a9691d8186/libs/go/log/fields.go
//
...
// Because our old API was a mirror of Zap's, we are able to type alias
// zap's field type and get relatively transparent support.  This saves us
// a lot of unnecessary wrapping and unwrapping in both code and at runtime,
// but it leaks Zap's field as part of our exported API.
//
// We might want to consider fixing this if we do want to support other
// backends transparently in future.
...
func (f msec) String() string {
	...
	if ms < 10 { // nolint: mnd
		...
		return fmt.Sprintf("%0.1fms", float64(us)/1000.0) // nolint: mnd
	}
...
func Float(key string, val float64, format string) Field {
	return zap.Stringer(key, floatFmt{val, format})
}
...
//   - stack trace from the first error that has one in the chain of wrapped errors starting from err in
//     `error.stack`, or [RichError] caller stack trace if no such stack trace was found.
...
// Group returns a field the wraps multiple sub fields.
// func Group(key string, fields ...Field) Field {
//	return groupField{key: key, fields: fields}
// }
...
func (p pct) String() string {
	return fmt.Sprintf("%0.3f%%", (p.part/p.whole)*100) // nolint: mnd
}
...
func Org(orgID int64) Field {
	return zap.Int64("org_id", orgID)
}

// Org32 writes an int32 orgID with the common key "org_id".
func Org32(orgID int32) Field {
	return zap.Int64("org_id", int64(orgID))
}

// traceField is used to log trace id and span ids.
...
// Deprecated: Use pkg/log/tracing Trace Logger instead
func (f traceField) MarshalLogObject(enc zapcore.ObjectEncoder) error {
	enc.AddUint64(traceIDKey, f.traceID)
	enc.AddUint64(spanIDKey, f.spanID)
	return nil
}

// TraceField logs the trace id and span id, when available in the context.

// Deprecated: Use pkg/log/tracing Trace Logger instead
func TraceField(ctx context.Context) Field {
	// XXX(jason): the easiest way to support this in zap is to have a "nested"
	// object;  in json, this can become actually nested.
	if span, ok := tracer.SpanFromContext(ctx); ok {
		sctx := span.Context()
		return zap.Object("", traceField{
			traceID: sctx.TraceID(), spanID: sctx.SpanID(),
		})
	}
	return zap.Skip()
}
